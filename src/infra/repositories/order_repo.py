...
class SQLAlchemyOrderRepository(OrderRepository):
    ...
    def find_by_id(self, order_id: uuid.UUID) -> Order | None:
        model = self._session.query(OrderModel).filter(OrderModel.id == str(order_id)).first()
        return self._to_domain(model) if model else None

    def find_by_user_id(
        self, user_id: uuid.UUID, status: str | None = None, page: int = 1, per_page: int = 20
    ) -> tuple[list[Order], int]:
        q = self._session.query(OrderModel).filter(OrderModel.user_id == str(user_id))
        if status:
    ...
    def list_all(
        ...
        q = self._session.query(OrderModel)
        if status:
            q = q.filter(OrderModel.status == status)
        q = q.order_by(OrderModel.created_at.desc())
        total = q.count()
    ...
    def _to_domain(self, model: OrderModel) -> Order:
        item_models = (
            self._session.query(OrderLineItemModel)
            .filter(OrderLineItemModel.order_id == model.id)
            .all()
        )
        return Order(
